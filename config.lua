...
local settingPrefix = "Mythic Plus Tweaks ";

local PAYPAL_TEXTURE = "|TInterface\\AddOns\\MythicPlusTweaks\\media\\paypal.tga:18|t";
local COFFEE_TEXTURE = "|TInterface\\AddOns\\MythicPlusTweaks\\media\\coffee.tga:18|t";
...
function Config:Init()
    ...
    self.category, self.layout = Settings.RegisterVerticalLayoutCategory("Mythic Plus Tweaks");
    ...
    do
        ...
        if modulesWithoutConfig[1] then
            ...
            local _, isExpanded = self:MakeExpandableSection("Basic Modules");
            local configBuilder = self:MakeConfigBuilder(self.db.modules, isExpanded);
            local openUIButton = configBuilder:MakeButton(
                "Open the Mythic+ UI",
                function() ns.Util:ToggleMythicPlusFrame(); end
            )
            configBuilder:SetEnableInitializer(openUIButton); -- not quite accurate, but whatever
...
do
    ...
    --- @param settingKey string|number
    --- @param tooltip string?
    --- @param options MPT_Config_SliderOptions
    ...
    --- @param settingKey string|number
    --- @param tooltip string?
    --- @param options MPT_Config_DropDownOptions|fun(): MPT_Config_DropDownOptions
...
--- @param moduleDb table
--- @param isExpanded fun(): boolean
--- @return MPT_ConfigBuilder configBuilder
...
do
    ...
    --- @param settingKey string|number
    --- @param tooltip string?
    --- @param options MPT_Config_SliderOptions # see Settings.CreateSliderOptions
    --- @param defaultValue number?
    ...
    function Config:MakeButton(label, onClick, tooltip)
        ...
        local initializer = Settings.CreateSettingInitializer('MythicPlusTweaks_SettingsButtonControlTemplate', data);
    ...
    function Config:MakeMultiButton(label, onClick, tooltip, buttonTexts)
        ...
        local initializer = Settings.CreateSettingInitializer('MythicPlusTweaks_SettingsMultiButtonControlTemplate', data);
        initializer:AddSearchTags(label);
        self.layout:AddInitializer(initializer);
    ...
    function Config:MakeColorPicker(label, settingKey, tooltip, defaultValue, dbTableOverride)
        ...
        local initializer = Settings.CreateSettingInitializer('MythicPlusTweaks_SettingsColorControlTemplate', data);
        self.layout:AddInitializer(initializer);
    ...
    function Config:MakeDonationPrompt(layout)
        self:MakeText("Addon development takes a large amount of time and effort. If you enjoy using Mythic Plus Tweaks, please consider supporting its development by donating. Your support helps ensure the continued improvement and maintenance of the addon. Thank you for your generosity!");

        local function onClick(_, buttonIndex)
            if buttonIndex == 1 then
                ns.Util:CopyText("https://www.paypal.com/cgi-bin/webscr?hosted_button_id=C8HP9WVKPCL8C&item_name=Mythic+Plus+Tweaks&cmd=_s-xclick");
            else
        ...
        return self:MakeMultiButton(
            "Donate",
            onClick,
            "If you enjoy using Mythic Plus Tweaks, consider supporting its development with a donation.",
            {
    ...
    MythicPlusTweaks_SettingsColorControlMixin = CreateFromMixins(SettingsControlMixin);
    do
        --- @class MPT_Config_ColorControlMixin
        local mixin = MythicPlusTweaks_SettingsColorControlMixin;
    ...
    MythicPlusTweaks_SettingsButtonControlMixin = CreateFromMixins(SettingsListElementMixin);
    do
        --- @class MPT_Config_ButtonControlMixin
        local mixin = MythicPlusTweaks_SettingsButtonControlMixin;
    ...
    MythicPlusTweaks_SettingsMultiButtonControlMixin = CreateFromMixins(SettingsListElementMixin);
    do
        --- @class MPT_Config_MultiButtonControlMixin
        local mixin = MythicPlusTweaks_SettingsMultiButtonControlMixin;
    ...
    MythicPlusTweaks_SettingsTextMixin = CreateFromMixins(DefaultTooltipMixin);
    do
        --- @class MPT_Config_TextMixin
        local mixin = MythicPlusTweaks_SettingsTextMixin;
